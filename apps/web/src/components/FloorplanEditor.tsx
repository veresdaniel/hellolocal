...
export function FloorplanEditor({ placeId, siteId }: FloorplanEditorProps) {
  ...
  const [draggedPinPosition, setDraggedPinPosition] = useState<{ x: number; y: number } | null>(null);
  ...
  useEffect(() => {
    ...
    if (rightPanelRef.current && isInstructionsOpen && rightPanelHeight === null && !currentIsMobile) {
      const timer = setTimeout(() => {
        if (rightPanelRef.current) {
          const fullHeight = rightPanelRef.current.getBoundingClientRect().height;
          setRightPanelHeight(fullHeight);
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      showToast(
        errorMessage || t("admin.errorUploading") || "Hiba a feltöltéskor",
        "error"
      );
  ...
  const handleDeleteFloorplan = async (id: string) => {
    const confirmed = await confirm({
      ...
      message: t("admin.floorplan.deleteConfirm") || "Biztosan törölni szeretnéd ezt az alaprajzot?",
  ...
  const handleImageClick = async (e: React.MouseEvent<HTMLImageElement> | { currentTarget: HTMLImageElement; clientX: number; clientY: number }) => {
    if (!selectedFloorplan || editingPin || isPanning || !imageRef.current || !containerRef.current) return;
  ...
  const handlePinLabelUpdate = useCallback(async (pinId: string, label: string, immediate = false) => {
    ...
    const performUpdate = async () => {
      ...
      try {
        ...
        setPins((prevPins) => prevPins.map((p) => (p.id === update.pinId ? { ...p, label: update.label } : p)));
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          {selectedFloorplan ? (
            ...
            <>
              ...
              >
                {isUploading ? t("common.uploading") || "Feltöltés..." : t("admin.floorplan.upload") || "Alaprajz feltöltése"}
              </button>
            </>
          )}
        </div>
      ...
      <div style={{ 
        ...
        <div
          ...
          style={{
            ...
            minHeight: isMobile 
              ? "300px" 
              : (rightPanelHeight ? `${rightPanelHeight}px` : "400px"),
            height: isMobile ? "auto" : (rightPanelHeight ? `${rightPanelHeight}px` : "400px"),
            touchAction: "none",
            overscrollBehavior: "contain",
            cursor: selectedFloorplan ? (isPanning ? "grabbing" : (scale > minScale && isSpacePressed ? "grab" : "default")) : "default",
            display: "flex",
            alignItems: "center",
            justifyContent: "center",
          }}
          ...
          {selectedFloorplan ? (
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        transition: isDragging && editingPin?.id === pin.id ? "none" : "left 0.1s, top 0.1s",
                      }}
            ...
            <div style={{
              ...
              >
                ...
                <rect x="20" y="20" width="80" height="80" stroke="currentColor" strokeWidth="2" fill="none" rx="4"/>
                <rect x="30" y="30" width="25" height="25" stroke="currentColor" strokeWidth="1.5" fill="none" rx="2"/>
                <rect x="65" y="30" width="25" height="25" stroke="currentColor" strokeWidth="1.5" fill="none" rx="2"/>
                <rect x="30" y="65" width="25" height="25" stroke="currentColor" strokeWidth="1.5" fill="none" rx="2"/>
                <rect x="65" y="65" width="25" height="25" stroke="currentColor" strokeWidth="1.5" fill="none" rx="2"/>
        ...
        <div 
          ...
          style={{ 
            ...
            height: isMobile ? "auto" : (rightPanelHeight && isInstructionsOpen ? `${rightPanelHeight}px` : undefined),
            minHeight: isMobile ? "auto" : (rightPanelHeight && isInstructionsOpen ? `${rightPanelHeight}px` : undefined),
          }}
        >
            ...
            >
              ...
              <div
                style={{
                  ...
                  padding: isInstructionsOpen ? `0 ${isMobile ? 12 : 16}px ${isMobile ? 12 : 16}px` : "0",
                }}
              >
                <ul style={{
                  margin: 0,
                  ...
                  <li>{t("admin.floorplan.instruction1") || "Kattints az alaprajzra egy új jelölő hozzáadásához"}</li>
                  <li>{t("admin.floorplan.instruction2") || "Húzd a jelölőket az egérrel a pozíciójuk módosításához"}</li>
                  <li>{t("admin.floorplan.instruction3") || "Kattints a jelölő címkéjére a jobb oldali listában az átnevezéshez"}</li>
                  <li>{t("admin.floorplan.instruction4") || "Használd az egér görgetését a nagyításhoz/kicsinyítéshez"}</li>
                  <li>{t("admin.floorplan.instruction5") || "Ctrl + egér vagy középső gomb + húzás a panorámázáshoz"}</li>
                </ul>
              </div>
            </div>
            ...
            <div style={{ 
              ...
              {selectedFloorplan ? (
                ...
                <div style={{
                  ...
                  {t("admin.floorplan.noPins") || "Még nincsenek jelölők. Kattints az alaprajzra egy hozzáadásához."}
                </div>
              ) : (
                <div 
                  style={{ 

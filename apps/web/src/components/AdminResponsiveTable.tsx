...
export function AdminResponsiveTable<T>({
  ...
  // Filter data
  const filteredData = searchQuery ? data.filter((item) => filterFn(item, searchQuery)) : data;
  ...
  if (isLoading) {
    return (
      <div style={{ minHeight: 400 }}>
        <TableSkeleton rows={5} columns={columns.length} isMobile={isMobile} />
      </div>
    );
  }
                ...
                onTouchEnd={(e) => {
                  ...
                  if (
                    ...
                    if (
                      ...
                      (diffX > QUICK_SWIPE_THRESHOLD &&
                        (quickSwipe || velocity > FAST_SWIPE_VELOCITY))
                    ) {
              ...
              >
                {filteredData.map((item, index) => {
                  ...
                  const isTransitioningIn =
                    displayOffset === 0 && displayPreviousOffset !== 0 && swipeDirection !== null;

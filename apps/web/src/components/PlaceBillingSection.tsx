...
import { getPlaceUpsellState, type PlaceUpsellState, type FeatureGate, getPlaceSubscription, updatePlaceSubscription, type PlaceSubscription, cancelSubscription, resumeSubscription, updateSubscription, getFloorplanEntitlement, getPlaceFeatureSubscriptions, type FloorplanEntitlement, type FeatureSubscription, cancelFeatureSubscription } from "../api/admin.api";
...
export function PlaceBillingSection({
  ...
  const [floorplanSubscription, setFloorplanSubscription] = useState<FeatureSubscription | null>(null);
  ...
  const renderFeatureGate = (gate: FeatureGate, label: string, icon: string, onClick?: () => void, isFloorplan: boolean = false) => {
    if (gate.state === "enabled") {
      ...
      if (isFloorplan && floorplanSubscription) {
        ...
        const planLabel = planLabels[floorplanSubscription.planKey] || floorplanSubscription.planKey;
        const billingLabel = billingLabels[floorplanSubscription.billingPeriod] || floorplanSubscription.billingPeriod;
        ...
        if (isExpired) {
          return (
            <div 
              ...
              onClick={onClick}
              onMouseEnter={onClick ? (e) => { e.currentTarget.style.background = "#fee2e2"; } : undefined}
              onMouseLeave={onClick ? (e) => { e.currentTarget.style.background = "#fef2f2"; } : undefined}
            >
        ...
        if (isExpiring && daysUntilExpiration !== null) {
          return (
            <div 
              ...
              onClick={onClick}
              onMouseEnter={onClick ? (e) => { e.currentTarget.style.borderColor = "#fb923c"; e.currentTarget.style.boxShadow = "0 6px 16px rgba(251, 146, 60, 0.3)"; } : undefined}
              onMouseLeave={onClick ? (e) => { e.currentTarget.style.borderColor = "#fed7aa"; e.currentTarget.style.boxShadow = "0 4px 12px rgba(251, 146, 60, 0.2)"; } : undefined}
            >
              ...
              <div style={{ 
                ...
                {(t("admin.floorplan.subscription.expiresIn") || "Lejár {days} nap múlva").replace("{days}", daysUntilExpiration.toString())}
              </div>
              <div style={{ 
        ...
        return (
          <div 
            ...
            onClick={onClick}
            onMouseEnter={onClick ? (e) => { e.currentTarget.style.background = "#dcfce7"; e.currentTarget.style.borderColor = "#16a34a"; } : undefined}
            onMouseLeave={onClick ? (e) => { e.currentTarget.style.background = "#f0fdf4"; e.currentTarget.style.borderColor = "#22c55e"; } : undefined}
          >
            ...
            <div style={{ 
              ...
              <div style={{ fontWeight: 600, marginBottom: 4 }}>{planLabel} • {billingLabel}</div>
              {periodEnd && (
    ...
    if (gate.state === "locked") {
      ...
      return (
        <div 
          ...
          onClick={onClick}
          onMouseEnter={onClick ? (e) => { e.currentTarget.style.background = "#fef3c7"; } : undefined}
          onMouseLeave={onClick ? (e) => { e.currentTarget.style.background = "#fff7ed"; } : undefined}
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
    ...
    if (gate.state === "limit_reached") {
      return (
        <div 
          ...
          onClick={onClick}
          onMouseEnter={onClick ? (e) => { e.currentTarget.style.background = "#fef3c7"; } : undefined}
          onMouseLeave={onClick ? (e) => { e.currentTarget.style.background = "#fffbeb"; } : undefined}
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <button
                  onClick={async () => {
                    const confirmed = await showConfirm({
                      ...
                      message: t("admin.subscription.suspendConfirm") || "Biztosan felfüggeszted az előfizetést?",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {upsellState && (
              <>
                ...
                {currentPlan !== "free" && renderFeatureGate(
                  // If we have an active floorplan subscription, force enabled state
                  floorplanSubscription && 
                  floorplanSubscription.status === "active" && 
                  (!floorplanSubscription.currentPeriodEnd || new Date(floorplanSubscription.currentPeriodEnd) > new Date())

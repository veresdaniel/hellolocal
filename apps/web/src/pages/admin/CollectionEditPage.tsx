...
function ItemsTab({
  ...
  const itemsIds = useMemo(() => items.map(i => i.id).join(','), [items]);
  const collectionItemsIds = useMemo(() => collection?.items?.map(i => i.id).join(',') || '', [collection?.items]);
  
  // Use ref to track previous itemsIds to avoid infinite loops
  const prevItemsIdsRef = useRef<string>('');
  
  // Reset local reordering flag when items prop changes significantly (e.g., after save/reload)
  // This runs AFTER the sync effect to reset the flag when server data matches our local changes
  useEffect(() => {
    // If items prop changed and we have local reordering, check if the order matches
    // If it matches, it means the server was updated with our changes, so reset the flag
    const sorted = [...items].sort((a, b) => a.order - b.order);
    const sortedIds = sorted.map(i => i.id).join(',');
    const currentItemsIds = currentItems.map(i => i.id).join(',');
    
    // If server order matches our local order, it means changes were saved
    if (hasLocalReorderingRef.current && sortedIds === currentItemsIds && sortedIds !== prevItemsIdsRef.current) {
      console.log("ItemsTab: local changes were saved, resetting reordering flag", {
        sortedIds,
        currentItemsIds,

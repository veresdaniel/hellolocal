...
export class AdminCollectionService {
  ...
  async findAll() {
    try {
      ...
      const sanitizedCollections = collections.map((collection: any) => ({
        ...collection,
        items: collection.items.filter((item: any) => item.site !== null),
        itemsCount: collection.items.filter((item: any) => item.site !== null).length,
      }));
  ...
  async reorderItems(collectionId: string, itemIds: string[]) {
    ...
    const collectionItemIds = new Set(collection.items.map((item: any) => item.id));

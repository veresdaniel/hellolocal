...
async function seedAnalytics(siteId: string, placeIds: string[], days: number, seed: number) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    await prisma.analyticsDaily.upsert({
      where: { day_siteId_placeId: { day, siteId, placeId: null } },
      update: {
        pageViews: baseViews,
        placeViews: Math.floor(baseViews * (0.65 + rand() * 0.2)),
        uniqueVisitors: uniq,
        ctaPhone: Math.floor(rand() * 15),
        ctaEmail: Math.floor(rand() * 8),
        ctaWebsite: Math.floor(rand() * 20),
        ctaMaps: Math.floor(rand() * 25),
      },
      create: {
        day,
        siteId,
